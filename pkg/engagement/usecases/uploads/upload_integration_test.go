...
func InitializeTestNewUploads(ctx context.Context) (*uploads.ImpUploads, infrastructure.Interactor, error) {
	infra := infrastructure.NewInteractor()
	uploads := uploads.NewUploads(infra)
	return uploads, infra, nil
}
...
func TestNewUploadsService(t *testing.T) {
	...
	f, i, err := InitializeTestNewUploads(ctx)
	if err != nil {
		t.Errorf("failed to initialize new Uploads: %v", err)
	}
	type args struct {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := uploads.NewUploads(tt.args.infrastructure); !reflect.DeepEqual(got, tt.want) {
				t.Errorf("NewUploadsService() = %v, want %v", got, tt.want)
			}
		})
	}
}
...
func TestUpload(t *testing.T) {
	ctx := context.Background()
	f, _, err := InitializeTestNewUploads(ctx)
	if err != nil {
		t.Errorf("failed to initialize new Uploads: %v", err)
	}
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			upload, err := f.Upload(ctx, tc.inp)
			if tc.expectError {
				assert.NotNil(t, err)
...
func TestServiceUploadImpl_FindUploadByID(t *testing.T) {
	...
	data, err := f.Upload(ctx, UploadInput)
	if err != nil {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := f.FindUploadByID(tt.args.ctx, tt.args.id)
