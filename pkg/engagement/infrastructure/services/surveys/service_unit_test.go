...
func TestUnit_RecordNPSResponse(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "happy case: successfully record NPS response",
			args: args{
				input: input,
			},
			wantErr: false,
		...
		{
			name:    "sad case: invalid record NPS response-empty input",
			args:    args{},
			wantErr: true,
			want:    false,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if tt.name == "happy case: successfully record NPS response" {
			...
			if tt.name == "sad case: invalid record NPS response-empty input" {
				fakeSurveysService.RecordNPSResponseFn = func(
					ctx context.Context,
					input dto.NPSInput,
				) (bool, error) {
